...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOD, nrf_CE_Pin|LD4_Pin|LD3_Pin|LD5_Pin
...
void readData(void) {
	uint16_t adcIn = 0;
		...
		adcIn=(uint16_t)rxData[0]<<8;
		adcIn=adcIn | rxData[1];
		sprintf(val, "%u\n",adcIn);
		lenght = strlen(val);
		HAL_UART_Transmit(&huart1, (uint8_t*)val, lenght, 100);
